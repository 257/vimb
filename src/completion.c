        ...
        CompletionSelectFunc selfunc, gboolean back)
{
    ...
    gtk_scrolled_window_set_child(GTK_SCROLLED_WINDOW(comp->win),
                                  GTK_WIDGET(comp->tree));

    gtk_box_append(GTK_BOX(gtk_widget_get_parent(GTK_WIDGET(c->statusbar.box))),
                   comp->win);
    ...
    GMainContext *ctx;
    while ((ctx = g_main_context_default()) && g_main_context_pending(ctx))
